...
- [Sources and references](#sources-and-references)
  - [ERD and schema design references](#erd-and-schema-design-references)
...
The implementation lives in `models.py` and `schema.sql` (DDL reference). Money is stored as **integer cents** (`price_cents`, `total_cents`, `unit_price_cents`) to avoid floating-point errors. **Order line items** snapshot `unit_price_cents` at checkout so historical orders stay correct if a book’s price changes later. External **ERD and schema design** references used while learning are listed under [ERD and schema design references](#erd-and-schema-design-references).
...
| `docs/legacy-code.md` | Small “before → after” code snapshots for review |
| `docs/fix-log.md` | Short fix narratives aligned with manual checklist rows (companion to fix commits on GitHub) |
...
|------|-------------------|----------------------------------------|-------------------------|
| **Functionality** | End-to-end behaviour: browse, auth, reviews, cart, checkout, orders, admin guards | **Automated:** `pytest` (route status, redirects, cart, admin 403/200, search, reviews). **Manual:** checklist **#1–#32** plus extra cases **#44–#46** (404 for bad id, empty search, order line items on Orders). | [Manual testing](#manual-testing) table; [Automated testing](#automated-testing) and [Testing summary table](#testing-summary-table); terminal log under [Running pytest locally](#running-pytest-locally-terminal-evidence) |
...
| 44 | Functionality | Open **`/books/999999`** (or any id with **no matching book**) | App returns **404** with the custom not-found page | Pass | Also covered by `test_book_detail_404` |  |
| 45 | Functionality | Search with `?q=` set to a **string that matches nothing** (e.g. `?q=zzznomatchbooks`) | Books list loads with **no matching titles** (empty grid / clear empty state) | Pass | Complements row **#4** (positive search) | [04b-search-no-results](docs/images/manual-testing/04b-search-no-results.png) |
| 46 | Functionality | On **Orders** (`/orders`), open a completed order | **Line items** show title, quantity, and line amounts; **order total** matches checkout | Pass | Confirms read path for `orders` + `order_items` | [19b-orders-page](docs/images/manual-testing/19b-orders-page.png) |
| 47 | Data mgmt | Add the **same book** to the cart **twice** (two separate “Add to cart” actions) | **One cart row** with **merged quantity** (not duplicate rows); subtotal correct | Pass | DB-backed cart merge | [16-cart-add](docs/images/manual-testing/16-cart-add.png) |
| 48 | Data mgmt | Put items in the cart, **log out**, then **log back in** as the **same user** | **Cart lines still present** (cart is stored per user in Postgres, not only in the browser tab) | Pass |  |  |
| 49 | Data mgmt | After **checkout** (row **#22**): open **Orders** and **Cart** again | **New order** appears with correct lines; **cart is empty** for that user | Pass | Multi-table write + cart clear | [19-checkout-success](docs/images/manual-testing/19-checkout-success.png)<br>[19b-orders-page](docs/images/manual-testing/19b-orders-page.png) |
| 50 | Data mgmt | After posting a **review**, **hard-refresh** the book detail page | Review text and rating **still show** (data read back from the database) | Pass |  | [12-review-created](docs/images/manual-testing/12-review-created.png) |
| 51 | Data mgmt | While logged in as **user B**, open the **edit URL** for **user A’s** review (if known) | **Blocked** with an error or forbidden response; **no edit** applied in the database | Pass | Server-side ownership guard | [15-review-owner-block](docs/images/manual-testing/15-review-owner-block.png) |
...
| 15 | Git / Deploy | The **GitHub repository did not connect reliably to Heroku**, and **commits could not be transferred** cleanly when moving work to a **new GitHub repo** (old remote / broken integration). There were **no YouTube videos** that matched this exact situation. **Claude** suggested trying **Cursor (free)** to diagnose Git and **push existing commits** to the new repository and fix the remote; after correcting `origin` and the Heroku Git remote, pushes and deployment worked again. | High | High | Re-point `git remote` at the new GitHub URL; use Cursor or terminal Git to verify `git push` reaches the right repo; reconnect Heroku to the new GitHub app/repo in the Heroku dashboard or via `heroku git:remote` as needed. | Resolved |
...
Quick index: [ERD and schema design references](#erd-and-schema-design-references) · [Feature resources](#feature-resources-inspiration--references) · [Flask](#flask) · [PostgreSQL (5 videos)](#postgresql-5-videos) · [Python (15 videos / playlists)](#python-15-videos--playlists) · [Sources for Python](#sources-for-python) · [Images used in this project](#images-used-in-this-project) · [Image credits](#image-credits)
...
These sources supported **entity–relationship thinking**, **cardinality**, and how the bookly schema maps to **SQLAlchemy** and **PostgreSQL** (including money and constraints). They are references only; the ERD and DDL in this repo were produced for this coursework.

- [Lucidchart — How to create an ER diagram](https://www.lucidchart.com/pages/er-diagrams) — Explanation of ER diagrams with examples and best practices.
- [GeeksforGeeks — Entity Relationship Diagram (ERD) introduction](https://www.geeksforgeeks.org/entity-relationship-diagram-erd-in-dbms/) — Overview of entities, relationships, and cardinality.
- [Vertabelo — Guide to ER diagrams](https://www.vertabelo.com/blog/entity-relationship-diagram-erd-tutorial/) — Step-by-step tutorial on designing ERDs.
- [SmartDraw — Database design for an online bookstore (sample ERD)](https://www.smartdraw.com/er-diagram/examples/online-bookstore.htm) — Example ERD for a bookstore, similar in spirit to this project.
- [SQLAlchemy documentation — Declarative models](https://docs.sqlalchemy.org/en/14/orm/declarative_tables.html) — Defining models and relationships in Python with SQLAlchemy.
- [PostgreSQL documentation — Constraints](https://www.postgresql.org/docs/current/ddl-constraints.html) — Foreign keys, constraints, and schema design guidance.
- [Martin Fowler — Money pattern](https://martinfowler.com/eaaCatalog/money.html) — Storing monetary amounts as integers and related design notes (bookly uses **integer cents** on `Book` and order lines).
- [Database Administrators Stack Exchange — How to store money in a database](https://dba.stackexchange.com/questions/107521/how-to-store-money-in-database) — Community discussion on money storage best practices.
